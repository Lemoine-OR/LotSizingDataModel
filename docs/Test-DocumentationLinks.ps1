...
foreach ($htmlFile in $htmlFiles) {
    ...
    foreach ($match in $attributeRegex.Matches($content)) {
        ...
        # Ignore anchors and non-local URI schemes.
        if (
        ...
        # Strip fragment and query string for filesystem validation.
        ...
        if ($pathPart.StartsWith([IO.Path]::DirectorySeparatorChar)) {
            $candidate = Join-Path $resolvedRoot $pathPart.TrimStart(
                [IO.Path]::DirectorySeparatorChar
            )
        }
        else {
        ...
        # Relative documentation links must remain inside the published site.
        if (
