...
{
    [Fact]
    public void ExecutableGlsp_UsesDedicatedFormulationNotStandardMilp()
    {
        ScientificClassificationResult classification =
            new ScientificClassificationEngine().Analyze(
                LotSizingProblemDescriptor.FromLegacyFeatures(
                    new LotSizingProblemFeatures
                    {
                        ...
                        SchedulingBucketMode = SchedulingBucketMode.MacroMicro,
        ...
        Assert.Equal(CanonicalLotSizingProblemClassId.GeneralLotSizingAndScheduling,
            classification.PrimaryProblemClass!.Definition.Id);
        Assert.Equal(LotSizingProblemClassSupportLevel.Executable,
            classification.PrimaryProblemClass.Definition.SupportLevel);

        ScientificFormulationCompatibilityResult dedicated =
            new ScientificFormulationCompatibilityService().Assess(
                classification,
                GlspSchedulingFormulation.FormulationIdValue);
        Assert.Equal(ScientificFormulationCompatibilityKind.Compatible, dedicated.Kind);

        ScientificFormulationCompatibilityResult standard =
            new ScientificFormulationCompatibilityService().Assess(
                classification,
                StandardLotSizingFormulation.StandardFormulationId);
        Assert.Equal(ScientificFormulationCompatibilityKind.Incompatible, standard.Kind);
    }
}
