...
public sealed class MacroMicroSchedulingStructuralValidator
{
        ...
        LotSizingSolution solution)
    {
        ...
        foreach (WorkCenterSchedulingDecision schedule
                 in solution.WorkCenterSchedulingDecisions)
        {
            WorkCenter? workCenter =
                instance.SupplyChain.Plants
                    .Where(plant =>
                        plant.Id == schedule.WorkCenter.PlantId)
                    .SelectMany(plant => plant.WorkCenters)
                    .SingleOrDefault(candidate =>
                        candidate.Id ==
                        schedule.WorkCenter.WorkCenterId);

            if (workCenter is null)
            {
                issues.Add(Error(
                    $"REFSCHED001: Unknown scheduling work center {schedule.WorkCenter}."));
                continue;
            }
            ...
            foreach (ProductionMicroPeriodDecision decision
                     in schedule.MicroPeriods)
            {
                int macro = decision.MicroPeriod.MacroPeriod;
                int micro = decision.MicroPeriod.MicroPeriodIndex;

                if (macro <= 0 ||
                    macro > instance.PlanningHorizon ||
                    micro <= 0 ||
                    micro > profile.MicroPeriodCount.GetCount(macro))
                {
                    issues.Add(Error(
                ...
                if (decision.SetupItemId > 0 &&
                    !instance.SupplyChain.Items.Any(item =>
                        item.Id == decision.SetupItemId))
                {
                    issues.Add(Error(
                        $"REFSCHED002: Unknown setup item {decision.SetupItemId} at {decision.MicroPeriod}."));
                }
                ...
                ProductionRouting? routing =
                    instance.SupplyChain.ProductionRoutings
                        .SingleOrDefault(candidate =>
                            candidate.Id == decision.RoutingId);
                ...
                if (decision.SetupItemId > 0 &&
                    routing.ItemId != decision.SetupItemId)
                {
                    issues.Add(Error(
                ...
                if (!routing.WorkCenters.Any(reference =>
                        reference.RefersToSameWorkCenter(
                            schedule.WorkCenter)))
                {
                    issues.Add(Error(
    ...
    private static SolutionCheckIssue Error(string message) =>
        new()
        {
