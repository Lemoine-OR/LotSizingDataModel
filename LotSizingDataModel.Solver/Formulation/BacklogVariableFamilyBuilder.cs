...
/// <summary>
/// Builds backlog-level variables for sourcing relationships
/// that explicitly define backlog constraints.
/// </summary>
public sealed class BacklogVariableFamilyBuilder :
    StandardLotSizingVariableFamilyBuilderBase
{
    /// <summary>
    /// Gets the backlog variable-family identifier.
    /// </summary>
    public override string FamilyId =>
        MathematicalDecisionCategory.Backlog;

    /// <summary>
    /// Determines whether backlog variables are enabled.
    /// </summary>
        ...
        StandardLotSizingFormulationOptions options)
    {
        ...
        {
            if (sourcing.BacklogConstraint is not null)
            {
                return true;
            }
        }
    ...
    /// <summary>
    /// Builds backlog-level variables.
    /// </summary>
            ...
            in instance.SupplyChain.DistributionCenterSourcings)
        {
                ...
                period++)
            {
                ...
                AddNonNegativeContinuousVariable(
                    context,
                    $"B_c{sourcing.DistributionCenterId}" +
                    $"_i{sourcing.ItemId}_w" +
                    $"{sourcing.Warehouse.ReferenceId}_t{period}",
                    domainKey,
                    upperBound,
                    $"Backlog level for distribution center " +
